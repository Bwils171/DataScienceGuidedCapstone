...
INNER JOIN Facilities as f
	ON b.facid = f.facid
WHERE Extract(DAY FROM starttime) = 14
	AND extract(YEAR_MONTH FROM starttime) =201209
HAVING cost > 30
ORDER BY cost DESC cost, and do not use any subqueries. */
...
SELECT concat(m.firstname, ' ', m.surname), f.name,
	CASE WHEN m.memid = 0 THEN (guestcost*slots)
		WHEN m.memid != 0 THEN (membercost*slots) 
		END AS cost
FROM (SELECT memid, facid, slots, starttime
      FROM Bookings
      WHERE Extract(DAY FROM starttime) = 14
	AND extract(YEAR_MONTH FROM starttime) =201209) AS b
INNER JOIN Members as m
	ON b.memid = m.memid
INNER JOIN Facilities as f
	ON b.facid = f.facid
...
SELECT f.name, strftime('%m', b.starttime), COUNT(strftime('%m', b.starttime))
    FROM Bookings as b
    INNER JOIN Facilities as f
    ON b.facid = f.facid
    WHERE memid != 0
